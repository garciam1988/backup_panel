// src/main/java/app/coincidir/api/domain/MemberDestinationTransferService.java
...
@Entity
@Table(
        name = "member_destination_transfer_service",
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "uq_member_destination_transfer_service_menu_item_member",
                        columnNames = {"menu_item_id", "member_id"}
                )
        },
        indexes = {
                @Index(name = "idx_member_dest_transfer_member", columnList = "member_id"),
                @Index(name = "idx_member_dest_transfer_menu_item", columnList = "menu_item_id")
        }
)
@Getter
@Setter
public class MemberDestinationTransferService {
    ...
    @JoinColumn(
            name = "menu_item_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_member_destination_transfer_service_menu_item")
    )
    ...
    @JoinColumn(
            name = "member_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_member_destination_transfer_service_member")
    )
